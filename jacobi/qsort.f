        subroutine QSORT(val,ind,stack,n)
...
c     on output val(1:n) is permuted such that its elements are in 
c               increasing order. ind(1:n) is an integer array which is
c               permuted in the same way as val(*).
c
c  code taken from SPARSKIT of Yousef Saad.
c  adapted by Matthias Bollhoefer 
c-----------------------------------------------------------------------
        doubleprecision valtmp, key
        integer indtmp, first, last,top, mid, j
c-----
        top=0
        first=1
        last =n
